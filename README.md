...
## The interactive human viewer

[`data/human_movie_viewer.html`](data/human_movie_viewer.html) is a three.js
viewer for the grow-from-one-cell human movie — genome → tetrapod → human on
one clock — with three views:

- **Movie** — the four phases (cloud morphogenesis → handoff → mesh maturation
  → anatomy reveal), with a per-frame panel of which genes are dialing in which
  layer (clock, axes, heads, proportions).
- **Gray's** — every named part of the model as its own isolable 3-D cloud,
  scored against its identity checklist, shown beside the matching plate from
  the public-domain 1918 edition of Gray's Anatomy (`data/grays_plates/`).
- **NCA+LLM** — the model internals: the heads (master-TF super-enhancer
  clusters), their cell masks, the inner-NCA per-fate parameters (Vm set-point,
  cadherin, PRC2 unlock), and the LGM adapter knobs with real GWAS β values.

The two tab datasets ship with the repository (`data/movie/grays_parts.json`,
`data/movie/nca_llm.json`; regenerated by `medic/grays_tab_export.py` and
`medic/nca_llm_tab_export.py`). The movie frames are a large generated
artifact (~58 MB) and are **not** committed — build them once, then serve:

```bash
python -m medic.human_movie              # writes data/movie/human_movie_frames.json
cd data && python -m http.server 8903
# open http://localhost:8903/human_movie_viewer.html
```

The viewer loads three.js from a CDN, so a browser with internet access is
